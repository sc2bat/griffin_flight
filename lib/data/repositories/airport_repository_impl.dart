...
class AirportRepositoryImpl implements AirportRepository {
  ...
  @override
  Future<Result<AirportDTO>> getAirportOne(int airportId) async {
    try {
      String url = '${Env.griffinGetUrl}/airports/';
      Map<String, dynamic> param = {"airport_id": airportId};

      final response = await fetchHttpWithParam(url: url, paramData: param);

      if (response.statusCode == 200) {
        final List<dynamic> jsonData = jsonDecode(response.body);
        List<AirportDTO> airportDTOList =
            jsonData.map((e) => AirportDTO.fromJson(e)).toList();
        if (airportDTOList.isNotEmpty) {
          return Result.success(airportDTOList[0]);
        }
        return const Result.error('airport data none');
      } else {
        return Result.error('response.statusCode => ${response.statusCode}');
      }
    } catch (e) {
